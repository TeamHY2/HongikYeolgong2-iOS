    ...
    var content: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .center, spacing: 0) {
                ...
                Button(action: {
                    router.push(to: .profile( nickname: userProfile.nickname, department: userProfile.department))
                }, label: {
                    Text("프로필 변경")
...
extension SettingView {
    ...
    func navigateToNotice() {
        router.push(to: .webView(title: "공지사항", url: SecretKeys.noticeUrl))
    }
    
    func navigateToInquiry() {
        router.push(to: .webView(title: "문의사항", url: SecretKeys.qnaUrl))
    }
}
